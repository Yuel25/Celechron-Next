...
void main() {
  group('moduleStatusesFromErrors', () {
    const labels = ['校历', '课表', '考试', '成绩', '作业', '实践'];
    ...
    test('null 为成功、「查询进行中」为进行中、其余为失败，且按下标对应标签', () {
      ...
      expect(statuses[4].state, FetchModuleState.failed);
      expect(statuses[5].label, '实践');
      expect(statuses[5].state, FetchModuleState.pending);
    });

    test('最终态（无「查询进行中」标记）只区分成功与失败', () {
      final statuses = moduleStatusesFromErrors(
          [null, null, '考试查询出错：500', null, null, null], labels);
      expect(
          statuses
